...
void loop() {
  Serial.println("ok");
  getIp();
  // sendAtCom(5000, "AT+CIFSR", ">", "ERROR", 5);
  delay(1500);
}
...
void gsmCheck(){
  ...
  while((gsmStatInt != 1)&&(gsmStatInt != 5)){
    ...
    Serial.print("gsm status =");Serial.println(gsmStatInt);
  }
}
uint8_t getGsmStat() {
  flushSim();
  ...
  Serial.print("tempGSM= ");Serial.println(tempGSM);
...
void getIp() {
  ...
  // String tempGSM = Serial2.readString();
  // Serial.print("IP= ");Serial.println(tempGSM);
  ...
  // Serial2.println("AT+CIICR");
  // tempGSM = Serial2.readString();
  // Serial2.println("AT+CIFSR");
  // String tempGSM1 = Serial2.readString();
  Serial.print("IP= ");Serial.println(tempGSM0);
}
